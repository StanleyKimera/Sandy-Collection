...
cd server && npm run dev      # API on http://localhost:3306
cd web && npm run dev         # shop screen on http://localhost:5173

# 3b. shop computer (one process, serves everything on port 3306)
cd web && npm run build
cd ../server && npm start
```

Then on the shop computer open `http://localhost:3306`.
On phones/tablets on the same Wi‑Fi open `http://<shop-computer-ip>:3306` — in Chrome or Safari
